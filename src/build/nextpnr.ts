 ...
 * `build/pnr/<top>.pnr.json` for gowin_pack, plus a JSON utilisation report
 * at `build/reports/pnr.json`.
 *
...
export interface NextpnrPlan {
	...
	/** Absolute path of the JSON utilisation/timing report this run writes. */
...
export function planNextpnr(
	...
	if (cstFiles.length === 0) {
		errors.push('Place & route needs a Gowin .cst constraint file; fpga.yaml lists none.');
	}
	...
	const reportPath = path.join(layout.reportDir, 'pnr.json');
	...
	const reportRelPath = toUnix(path.relative(projectRoot, reportPath));
	...
	const clockMhz = board.clocks[0]?.mhz;
	if (clockMhz && clockMhz > 0) {
		args.push('--freq', String(clockMhz));
	}
	args.push('--json', netlistInRelPath, '--write', pnrJsonRelPath, '--report', reportRelPath);
